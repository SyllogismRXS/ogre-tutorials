// In this program, we will just aim to show a window.
// This is done in a few steps. Please note you don't need to understand all that to make a very good program.
// These informations are provided in case you are interested to know how to do things from scratch.
// The visual content of the window may be black, or it may contains ghosts of previously displayed contents.
// The visual content of a window that is not explicity redrawn is undefined (often depends on the graphic card driver).
...
int main()
{
     ...
     try
     {
          ...
          Ogre::String mPluginsCfg = "../share/dist/config/plugins.cfg";

          std::auto_ptr<Ogre::Root> lRoot(
               new Ogre::Root(mPluginsCfg, lConfigFileName, lLogFileName));
